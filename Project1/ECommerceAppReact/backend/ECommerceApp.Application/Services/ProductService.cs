...
public class ProductService : IProductService
{
    ...
    public async Task<ProductDetailsDto?> GetProductDetailsAsync(int id, bool includeInactive = false)
    {
        var product = await _productRepository.GetProductWithDetailsAsync(id);
        if (product == null || (!product.IsActive && !includeInactive))
            return null;
    ...
    public async Task<ProductDto> CreateProductAsync(CreateProductDto dto, string sellerId)
    {
        ...
        // Fallback if re-fetch fails (common in some EF/Oracle scenarios immediately after insert)
        ...
        return new ProductDto
        {
            ...
            SellerName = "You (Reload to see details)", // Placeholder as we don't have easy access to user repo here
            IsActive = product.IsActive,
            CreatedAt = product.CreatedAt
        };
    }
